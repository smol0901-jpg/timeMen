...
export function fmtMin(m: number): string {
  ...
  return `${p2(Math.floor(x / 60))}:${p2(x % 60)}`;
}
...
export function fmtDurH(min: number): string {
  const m = Math.max(0, Math.round(min));
  return `${Math.floor(m / 60)}:${p2(m % 60)}`;
}
export function hDec(min: number): number {
...
export function monthEnd(key: string): string {
  return key.slice(0, 8) + p2(daysInMonth(key));
}
...
export function weekdayIdx(key: string): number {
  return (parseKey(key).getDay() + 6) % 7; // Пн=0
}
export function isWeekend(key: string): boolean {
  return weekdayIdx(key) >= 5;
}
...
export function fmtDM(key: string): string {
  const [, m, d] = key.split("-").map(Number);
  return `${WD[weekdayIdx(key)]}, ${d} ${MONTHS[m - 1].slice(0, 3)}`;
}
...
export function rint(r: () => number, a: number, b: number): number {
